...
public class AuthenticationServiceApplicationTests {
	 ...
	 * //Controller
	 * 
	 * @Test public void getMentor() {
	 * 
	 * Mentor mentorNew=new Mentor(); mentorNew.setUserID("John");
	 * mentorNew.setContactNo("8806756798");
	 * mentorNew.setEmail("john@email.com");
	 * mentorNew.setYearsOfExperience((float) 10);
	 * mentorNew.setFirstName("john"); mentorNew.setLastName("mathew");
	 * mentorNew.setLinkedInUrl("john@linked.com");
	 * mentorNew.setPassword("pwd"); mentorNew.setId(5L);
	 * 
	 * 
	 * 
	 * List<Mentor> mentorList = new ArrayList<>(); mentorList.add(mentorNew);
	 * when(userController.getMentor()).thenReturn(mentorList);
	 * 
	 * //test List<Mentor> testMentor=userController.getMentor();
	 * assertEquals(1, testMentor.size()); for(Mentor mentors : testMentor) {
